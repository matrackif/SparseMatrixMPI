...
void parallelMult(int rank, int size, matrix2D<T> & m1, matrix2D<T> & m2)
{
	...
	int numRowsPerProcess = m1RowCount / size;
	if (rank == 0)
	{
		...
		std::cout << "offset: " << offset << " numRowsPerProcess: " << numRowsPerProcess << std::endl;
		//std::cout << "matrix m1: \n" << m1 << std::endl;
		//std::cout << "matrix m2: \n" << m2 << std::endl;
		...
		{
			MPI_Send(m1.getData(offset + (d * numRowsPerProcess)), m1ColCount * numRowsPerProcess, MPI_DOUBLE, d + 1, tag, MPI_COMM_WORLD);
			MPI_Send(m2.getData(0), m2.getNumElements(), MPI_DOUBLE, d + 1, tag, MPI_COMM_WORLD); // TODO partition it differently
		}
		m1SubMatrix = m1.createSubMatrix(0, offset - 1, 0, m1ColCount - 1);
		...
		result.copyValues(partialResult, 0, offset - 1);
		std::cout << "Rank=" << rank << " copyValues() done" << std::endl;
		for (int i = 0; i < size - 1; i++)
		{
			MPI_Recv(result.getData(offset + (i * numRowsPerProcess)), numRowsPerProcess * m2ColCount, MPI_DOUBLE, i + 1, tag, MPI_COMM_WORLD, &status);
		}
...
void parallelAdd(int rank, int size, matrix2D<T> & m1, matrix2D<T> & m2)
{
	...
	if (m1RowCount != m2RowCount || m1ColCount != m2ColCount)
	{
		if (rank == 0)
		{
			std::cerr << "parallelAdd() Error: Dimensions msut be equal" << std::endl;
		}
		return;
	}
	...
	if (rank == 0)
	{
		...
		{
			MPI_Send(m1.getData(offset + (d * numRowsPerProcess)), m1ColCount * numRowsPerProcess, MPI_DOUBLE, d + 1, tag, MPI_COMM_WORLD);
			MPI_Send(m2.getData(offset + (d * numRowsPerProcess)), m2ColCount * numRowsPerProcess, MPI_DOUBLE, d + 1, tag, MPI_COMM_WORLD); // TODO partition it differently
		}
		m1SubMatrix = m1.createSubMatrix(0, offset - 1, 0, m1ColCount - 1);
		m2SubMatrix = m2.createSubMatrix(0, offset - 1, 0, m2ColCount - 1); 
		std::cout << "Rank=" << rank << " m1 submatrix: \n" << m1SubMatrix << std::endl;
		std::cout << "Rank=" << rank << " m2 submatrix: \n" << m2SubMatrix << std::endl;
